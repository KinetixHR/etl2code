#
#
# This script is to be run to fix data in dw2_jobs. You need the job id's that are broken in order to use this script, as well as a date range to run it on
#
#
...
import urllib
import logging
logging.basicConfig(filename='alljobs_fixes_logging.log', level=logging.INFO,
...
def end_dating(df):
    ...
    for el in df.iterrows():
        job_id = str(el[1]["JOB_ID"])
        job_id = "'" + job_id + "'"
        sql_statement = f"""SELECT JOB_ID FROM dw2_jobs WHERE JOB_ID = '{job_id}'"""
        logging.info(sql_statement)
        df_dw2jobs_search = pd.read_sql(sql_statement,conn)
        data_exists_in_db = df_dw2jobs_search.shape[0]

        if data_exists_in_db == 0:
            df.at[el[0],"END_DATE"] = '12/31/9999'
        else:
            df.at[el[0],"END_DATE"] = start_dates_for_script
        
        df.at[el[0],"END_DATE"] = pd.to_datetime(df["END_DATE"],errors = 'coerce')
        df.at[el[0],"END_DATE"] = df["END_DATE"].dt.strftime('%Y-%m-%d')
        logging.info(data_exists_in_db)
    
    return df

# Ask for data from user: 
input_list_of_job_ids = input("Please provide a comma seperated list of JOB_IDs to be processed")
input_list_of_job_ids = input_list_of_job_ids.split(",")
input_list_of_job_ids = str(input_list_of_job_ids).replace("[","").replace("]","")

start_dates_for_script = input("Please provide the start date for the script to look for data to process")
end_dates_for_script = input("Please provide the end date for the script to look for data to process")

date_format = "%m/%d/%Y"
end_dates_for_script =dt.datetime.strptime(end_dates_for_script,date_format).strftime("%Y-%m-%d")
start_dates_for_script = dt.datetime.strptime(start_dates_for_script,date_format).strftime("%Y-%m-%d")

logging.info(input_list_of_job_ids)
logging.info(start_dates_for_script)
logging.info(end_dates_for_script)
...
FROM TR1__Job__c
WHERE (((LastModifiedDate > {start_dates_for_script}T05:00:00Z AND LastModifiedDate < {end_dates_for_script}T05:00:00Z) OR (CreatedDate > {start_dates_for_script}T05:00:00Z AND CreatedDate < {end_dates_for_script}T05:00:00Z)) AND Id IN ({input_list_of_job_ids}))"""
...
if ALLJOBS_SUCCESS_FLAG is True:
    try:
        ...
        logging.info("transform_data() exited successfully")
        df_alljobs = end_dating(df_alljobs)
        logging.info("end_dating() exited successfully")
        
        logging.info(
            "Loaded in all jobs from API and transformed columns...: %s",str(df_alljobs.shape))
        logging.info(df_alljobs.columns)
        logging.info(df_alljobs)
...
if ALLJOBS_SUCCESS_FLAG == True:
    try: 
        df_alljobs.to_sql('dw2_jobs', con=engine, if_exists='append', index=False)

...
df_existing = pd.read_sql("SELECT * FROM dbo.dw2_jobs",con = engine)
...
update_statement = update(your_table).where(your_table.c.JOB_ID.in_(ids_to_update)).values(END_DATE=new_value)

...
def compare_and_find_updated_reqs(req_list_,new_dataframe,existing_dataframe):
    ...
    new_dataframe_last_modified_info = new_dataframe[["JOB_ID","DAYS_OPEN","LAST_MODIFIED_BY_ID","LAST_MODIFIED_DATE_DT","LAST_MODIFIED_DATE_TS"]]
    new_dataframe = new_dataframe.drop(columns =["DAYS_OPEN","LAST_MODIFIED_BY_ID","LAST_MODIFIED_DATE_DT","LAST_MODIFIED_DATE_TS"])
    existing_dataframe = existing_dataframe.drop(columns =["DAYS_OPEN","LAST_MODIFIED_BY_ID","LAST_MODIFIED_DATE_DT","LAST_MODIFIED_DATE_TS"])
    ...
    data_to_return.columns = ['JOB_ID', 'ACCOUNT_MANAGER', 'ACTION_PLAN', 'COUNT_NET_LONG_LIST',
       'AFFILIATE_VENDORS', 'PRIMARY_JOB', 'BUDGETED_START_DATE',
       ...
       'HIRING_MANAGER', 'INTAKE_COMPLETED', 'INTAKE_COMPLETED_DATE',
       'INTAKE_COMPLETED_TIMELINE', 'INTERNAL_ROLE', 'INVOICE_PAID',
       'INVOICE_PAID_DATE', 'JOB_FAMILY', 'NAME', 'JOB_NUMBER', 'JOB_OWNER',
       'JOB_STAGE_TEXT', 'LAST_ACTIVITY_DATE', 'LEVEL', 'MAXIMUM_PAY_RATE',
       'MINIMUM_PAY_RATE', 'OPEN_DATE', 'OWNER_ID', 'PAYGRADE', 'PIPELINE_JOB',
       'PO_NUMBER', 'POINT_VALUE', 'POST_EXTERNALLY', 'PRIMARY_JOB_REQ',
       'PRIMARY_SECONDARY', 'PROJECT', 'RECORD_TYPE_NAME', 'REGIONAL_AREA',
       'SALARY_LOW', 'SALARY_MID', 'SHIFT_INFORMATION', 'STATE_AREA', 'STATUS',
       'TARGETED_BILLING_DATE', 'COUNT_TOTAL_OFFERS', 'COUNT_TOTAL_SUBMITTALS',
       'VARIABLE_PROJECT_FTE', 'CREATED_DATE_DT', 'CREATED_DATE_TS',
       'EFFECTIVE_DATE', 'END_DATE', 'DAYS_OPEN', 'LAST_MODIFIED_BY_ID',
       'LAST_MODIFIED_DATE_DT', 'LAST_MODIFIED_DATE_TS']
...
def transform_data(df):
    ...
    df["TARGETED_BILLING_DATE"] = pd.to_datetime(df["TARGETED_BILLING_DATE"])
    df['TARGETED_BILLING_DATE'] = df["TARGETED_BILLING_DATE"].dt.strftime('%Y-%m-%d')
...
def transform_dw2_data(df):
    ...
    df.columns = ['JOB_ID', 'ACCOUNT_MANAGER', 'ACTION_PLAN', 'COUNT_NET_LONG_LIST',
       'AFFILIATE_VENDORS', 'PRIMARY_JOB', 'BUDGETED_START_DATE',
       'CERTIFICATION', 'EXTERNAL_CHALLENGES', 'INTERNAL_CHALLENGES', 'CITY',
       'INDUSTRY', 'CLIENT_REQ_NUMBER', 'CLOSED_DATE', 'CLOSED_REASON',
       'COACH', 'COMPANY', 'COMPANY_NAME', 'COMPENSATION_COMMENTS', 'CONTACT',
       ...
       'INTAKE_COMPLETED_TIMELINE', 'INTERNAL_ROLE', 'INVOICE_PAID',
       'INVOICE_PAID_DATE', 'JOB_FAMILY', 'NAME', 'JOB_NUMBER', 'JOB_OWNER',
       'JOB_STAGE_TEXT', 'LAST_ACTIVITY_DATE', 'LAST_MODIFIED_BY_ID', 'LEVEL',
       'MAXIMUM_PAY_RATE', 'MINIMUM_PAY_RATE', 'OPEN_DATE', 'OWNER_ID',
       'PAYGRADE', 'PIPELINE_JOB', 'PO_NUMBER', 'POINT_VALUE',
       'POST_EXTERNALLY', 'PRIMARY_JOB_REQ', 'PRIMARY_SECONDARY', 'PROJECT',
       'RECORD_TYPE_NAME', 'REGIONAL_AREA', 'SALARY_LOW', 'SALARY_MID',
       'SHIFT_INFORMATION', 'STATE_AREA', 'STATUS', 'TARGETED_BILLING_DATE',
       'COUNT_TOTAL_OFFERS', 'COUNT_TOTAL_SUBMITTALS', 'VARIABLE_PROJECT_FTE',
       'CREATED_DATE_DT', 'CREATED_DATE_TS', 'LAST_MODIFIED_DATE_DT',
       'LAST_MODIFIED_DATE_TS', 'EFFECTIVE_DATE', 'END_DATE']
    logging.info("Done with changing column names...")

    logging.info("Starting date transformations...")
    df["INTAKE_COMPLETED_DATE"] = pd.to_datetime(df['INTAKE_COMPLETED_DATE'])
...
df_existing = transform_dw2_data(df_existing)
logging.info(f"shape of data from dw2jobssandbox = {df_existing.shape}")
...
updated_req_df.to_csv('extract.csv')
...
try:
    ...
    updated_req_df.to_sql('dw2_jobs_dev',con = engine, if_exists = 'append', index= False)
